...
export const AdminDashboard = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      {isAuthenticated && (
        <div className="w-full md:w-64 glass border-b md:border-b-0 md:border-r border-white/10 p-4 md:p-8 flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start gap-4 md:gap-10 shrink-0">
          <div className="flex items-center group cursor-pointer">
            <Logo className="w-12 h-12 md:w-24 md:h-24 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500" />
          </div>
          
          <nav className="hidden md:flex flex-1 space-y-2 w-full">
            {[
          ...
          <button 
            ...
            className="flex items-center gap-2 text-red-400 hover:text-red-300 transition-colors text-[10px] md:text-sm font-bold uppercase tracking-widest"
          >
      ...
      {/* Main Content */}
      <div className="flex-1 overflow-y-auto p-4 md:p-12 relative bg-[#050505]">
        <button onClick={onClose} className="absolute top-4 right-4 md:top-8 md:right-8 text-white/20 hover:text-gold transition-all z-20">
        ...
        {!isAuthenticated ? (
          ...
          <div className="space-y-12 max-w-6xl">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8">
              <div>
                <h1 className="text-3xl md:text-5xl font-bold tracking-tighter mb-2">Dashboard</h1>
                <p className="text-white/40 text-[10px] md:text-sm italic">Welcome back to the command center.</p>
              </div>
              <div className="flex items-center gap-2 md:gap-3">
                <button className="flex items-center gap-2 glass px-4 md:px-6 py-2 rounded-full text-[8px] md:text-[10px] font-bold uppercase tracking-widest hover:bg-white/5 transition-all"><Filter size={12} /> Filter Reports</button>
                <div className="text-[8px] md:text-[10px] glass px-4 md:px-6 py-2 rounded-full text-green-400 font-bold uppercase tracking-widest border-green-500/20">Live</div>
              </div>
            </div>
